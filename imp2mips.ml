...
let tr_function fdef =
  ...
  | Cst _ | Bool _ | Var _ | Call (_, _) -> 1
  | Binop(_, e1, e2) -> 1 + nb_reg_expr e1 + nb_reg_expr e2 
in
  let max_reg_expr e1 e2 =
   if nb_reg_expr e1 > nb_reg_expr e2 then (e1, e2) else (e2, e1)
in
  let rec tr_expr i e = match e with
    ...
    | Binop(bop, e1, e2) ->
       let op = match bop with
         ...
         | Lt  -> slt
       in
       let (e1, e2) = max_reg_expr e1 e2 in
       (* Evaluate [e2] *)
       tr_expr i e2
       (* Evaluer [e1] *)
       @@ tr_expr (i+1) e1
    ...
    | Call(f, params) ->
      (* try to save registers before call *)
       ...
       let params_code =
         List.fold_right
           (fun e code -> code @@ tr_expr i e @@ push regs.(i))
           params nop
       in
       params_code
