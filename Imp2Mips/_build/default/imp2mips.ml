...
let tr_function fdef =
  ...
  let rec tr_expr e i = match e with
    ...
    | Cst(n)  -> li t.(i) n
    | Bool(b) -> if b then li t.(i) 1 else li t.(i) 0
    ...
    | Binop(bop, e1, e2) ->
       ...
       (* Evaluate [e2] *)
       tr_expr e2 i
       (* Store the value of [e2] on the stack (it has been put in $ti). *)
       @@ push t.(i)
       (* Evaluer [e1] *)
       @@ tr_expr e1 i
       (* Retrieve the value of [e2] that has been waiting on the stack,
          put it in a register other than $ti (that one contains the value
          of [e1]). *)
       @@ pop t.(i+1)
    ...
    | Call(f, params) ->
       ...
       let params_code =
         List.fold_right
           (fun e code -> code @@ tr_expr e i @@ push t.(i))
           params nop
       in
       params_code
       ...
       (* After the call, execution comes back at this point of the code, and
          register $ti contains the returned value. *)
